...
class CalendarPage(tk.Frame):
    ...
    # canvas.place(x = 0, y = 0)
        ...
        # 元件內容
        lbl_calendar = Label(root, text = "行事曆", font = f1) 
        lbl_admin = Label(root, text = admin_content, font = f2, borderwidth = 2, relief = "ridge", wraplength = 300, justify = 'left')
        btn_return = Button(root, text = "返回", command = self.clickBtnReturn, width = 7,height = 2, font = f2, bg = 'Lavender')
...
class Day1Page1(object):  # Day 1 小知識的日子
    ...
    def openCalendar(self):
        newWindow = Toplevel()
        labelExample = Label(newWindow, text = "行事曆(待套入)")
        labelExample.pack()
